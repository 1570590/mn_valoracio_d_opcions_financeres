"""
Generació dels gràfics per l'equacio W amb el canvi de variable revertit
"""
import os
...
def grafic_W_R(
    ...
    """
    Funció per a crear i guardar el gràfic de la solució W(R, t) en 3D i
    en 2D utilitzant l'esquema explícit o Crank-Nicolson.
    ...
    -------
    None
        Aquesta funció guarda els gràfics generats en arxius en el directori especificat.
    ...
    carpeta, ruta_fitxer_2d = path_grafic(esquema, "W(R,t)", nom_fitxer_2d)
    ...
    ax1.plot_surface(R_grid, T_grid, W.T, cmap="viridis", alpha=0.5)
    ax1.set_xlabel("R")
    ax1.set_ylabel("t")
    ax1.set_zlabel("W(R,t)")
    if opcio == "call":
        ax1.set_title("Solució de W(R,t) per una opció de compra")
    elif opcio == "put":
        ax1.set_title("Solució de W(R,t) per una opció de venda")
    ...
    # Fem la gràfica de l'evolució de W per a diversos valors de t
    step = max(len(t) // 3, 1)
    ...
    for t_idx in range(0, len(t), step):
        ...
        (line,) = ax2.plot(R, W[:, t_idx], label=f"t = {t_invertit[t_idx]:.2f}")
        handles.append(line)  # Guardem la línia per a la llegenda

    ax2.set_xlabel("R")
    ax2.set_ylabel("Valor de W")
    if opcio == "call":
        ax2.set_title(
            "Evolució de W(R,t) per una opció de compra per a diferents valors de t"
        )
    elif opcio == "put":
        ax2.set_title(
            "Evolució de W(R,t) per una opció de venda per a diferents valors de t"
        )
