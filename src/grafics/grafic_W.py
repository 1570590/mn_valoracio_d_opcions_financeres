"""
Generació dels gràfics per l'equacio W
"""
import os
...
def grafic_W(
    ...
    """
    Funció per a crear i guardar el gràfic de la solució W(x, tau)
    en 3D i en 2D utilitzant l'esquema explícit o Crank-Nicolson.
    ...
    -------
    None
        Aquesta funció guarda els gràfics generats en arxius
    ...
    carpeta, ruta_fitxer_2d = path_grafic(esquema, "W(x,tau)", nom_fitxer_2d)
    ...
    ax1.plot_surface(X, T, W.T, cmap="viridis", alpha=0.5)
    ax1.set_xlabel("x")
    ax1.set_ylabel("τ")
    ax1.set_zlabel("W(x,τ)")
    if opcio == "call":
        ax1.set_title("Solució de W(x,τ) per una opció de compra")
    elif opcio == "put":
        ax1.set_title("Solució de W(x,τ) per una opció de venda")
    ...
    # Fem la gràfica de l'evolució de W per a diversos valors de tau
    step = max(len(tau) // 3, 1)
    for t_idx in range(0, len(tau), step):
        ax2.plot(x, W[:, t_idx], label=f"τ = {tau[t_idx]:.2f}")

    ax2.set_xlabel("x")
    ax2.set_ylabel("Valor de W")
    if opcio == "call":
        ax2.set_title(
            "Evolució de W(x,τ) per una opció de compra per a diferents valors de τ"
        )
    elif opcio == "put":
        ax2.set_title(
            "Evolució de W(x,τ) per una opció de venda per a diferents valors de τ"
        )
